...
def classify(job,response,transport,evidence,identity,retry_exhausted):
 ...
 if evidence.get('platform')=='Apple Podcasts' and evidence.get('requestedEpisodeId'):
  reasons=set(evidence.get('reviewReasons') or [])
  if 'resolved_to_different_episode' in reasons:return ('reachable_unconfirmed','resolved_to_different_episode','resolution_mismatch')
  if 'metadata_episode_id_conflict' in reasons:return ('reachable_unconfirmed','metadata_episode_id_conflict','resolution_mismatch')
  if not evidence.get('destinationMatchesRequestedEpisode'):
   reason='canonical_url_dropped_episode_identifier' if 'canonical_url_dropped_episode_identifier' in reasons else 'requested_episode_not_confirmed'
   return ('reachable_unconfirmed',reason,'resolution_mismatch')
...
def run_one(job,cfg):
 ...
 if response is not None:
  setattr(response,'requested_url',job.get('initialUrl') or url); setattr(response,'normalized_url',url)
  ...
  apple_episode_confirmed=evidence.get('platform')!='Apple Podcasts' or evidence.get('destinationMatchesRequestedEpisode')
  identity=compare_identity(job,evidence) if apple_episode_confirmed else {'status':'insufficient_evidence','identityComparisonSkipped':True,'skipReason':'apple_requested_episode_not_confirmed','warnings':['repository_identity_comparison_skipped'],'expected':{'series':job.get('podcastSeries'),'title':job.get('episodeTitle'),'episodeNumber':job.get('episodeNumber'),'publicationDate':job.get('publicationDate'),'guests':job.get('expectedGuests',[]),'platform':job.get('platform')},'captured':{'series':evidence.get('showTitle'),'title':evidence.get('episodeTitle'),'episodeNumber':evidence.get('episodeNumber'),'publicationDate':evidence.get('publicationDate'),'platform':evidence.get('platform'),'requestedEpisodeId':evidence.get('requestedEpisodeId'),'resolvedEpisodeId':evidence.get('resolvedEpisodeId'),'canonicalEpisodeId':evidence.get('canonicalEpisodeId'),'metadataEpisodeId':evidence.get('metadataEpisodeId')}}
