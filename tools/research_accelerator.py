...
from podcast_classification_engine import classify_release
...
def make_draft(item, page, entities, cfg, transcript=''):
    ...
    classification=classify_release({
      'title':page.get('title',''),'episodeTitle':ep_title,'description':page.get('description',''),
      'bodyText':page.get('bodyText',''),'transcript':transcript,'summary':page.get('description','')
    },mentions)
    url=page.get('url','')
    ...
    draft={
      ...
      'researchNotes':['Generated by GreyAlien Research Accelerator; every classification, relationship, and role requires human review.'],
    ...
    review={'id':eid,'sourceUrl':url,'confidenceScore':score,'confidenceBand':'high' if score>=80 else 'medium' if score>=55 else 'low',
            'classification':classification,
            'matchedExistingEntities':mentions,'unresolvedCandidatePhrases':classification['entityExtraction']['candidateNames'], 'warnings':reasons+classification['warnings'],
            'requiredReview':['Approve or revise Content Class and Episode Type','Approve Primary and Secondary Topics','Verify title and publication date','Classify each matched person as guest, host, or discussed subject','Remove false-positive entity matches','Create new canonical entities only when source-supported and materially relevant','Apply the selected Research Accelerator depth','Draft 3–7 principal claims only for approved Research Episodes when the episode materially advances claims','Confirm Official Links remain entity-owned and episode URLs stay in Reference Sources']}
    return draft,review
...
def main():
    ...
    for item in cfg.get('episodes',[]):
        ...
        md=[f'# Review — {draft["name"]}','',f'- Source confidence: **{review["confidenceScore"]}/100 ({review["confidenceBand"]})**',f'- Classification confidence: **{c["confidence"]}/100**',f'- Content class: **{c["contentClass"]}**',f'- Episode type: **{c["episodeType"]}**',f'- Research depth: **{c["researchAccelerator"]}**',f'- Primary topics: {", ".join(c["primaryTopics"]) or "Manual review required"}',f'- Secondary topics: {", ".join(c["secondaryTopics"]) or "None proposed"}',f'- Source mode: `{source_mode}`',f'- Official page: {url or "Not supplied"}','','## Existing entity matches']
